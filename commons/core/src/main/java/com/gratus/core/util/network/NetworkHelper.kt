...
fun getNetworkErrorMessage(responseBody: ResponseBody?): String {
    return try {
        ...
        when {
            ...
            else -> Resources.getSystem().getResourceName(R.string.generic_error)
        }
    } catch (e: Exception) {
        Resources.getSystem().getResourceName(R.string.generic_error)
    }
}
...
fun getErrorMessage(errorType: String): String {
    return try {
        when (errorType) {
            SESSION_EXPIRED -> {
                Resources.getSystem().getResourceName(R.string.connection_time_out)
            }
            TIMEOUT -> {
                Resources.getSystem().getResourceName(R.string.connection_time_out)
            }
